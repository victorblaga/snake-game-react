...
function Grid({ halfSize, tickDuration }) {
    ...
    const resetMovementTimer = function (direction) {
        ...
        const onTick = function () {
            ...
            console.log(`FoodRef location: row = ${foodRef.current.row}; col = ${foodRef.current.col}`);
            moveSnake(direction);
        }
    ...
    for (let i = 0; i < fullSize; i++) {
        ...
        for (let j = 0; j < fullSize; j++) {
            ...
            const cell = <div className={cellClasses.join(" ")} data-col={columnIndex} data-row={rowIndex} key={key}></div>;
            cells.push(cell);
        }

        const row = <div className="row" data-row={rowIndex} key={rowIndex}>{cells}</div>;
        rows.push(row);
    }
    ...
    return <div className="wrapper">
        ...
        {gameOver ? <div className="status">Game over. Press Space to restart!</div> : null}
    </div>;
}
